...
def check_file(file_path: str, file_escape_size: float) -> tuple[bool, str, str]:
    """
    检查文件是否有效.

    Returns:
        tuple[bool, str, str]: 是否有效; 文件名, 无效时更新 outline; 文件路径, 无效时更新 tag
    """
    if os.path.islink(file_path):
    ...
    if not os.path.exists(file_path):
        ...
        outline = split_path(file_path)[1]
        tag = file_path
        return False, outline, tag
...
def move_trailer_video(folder_old_path: str, folder_new_path: str, file_name: str, naming_rule: str) -> None:
...
def move_torrent(folder_old_path: str, folder_new_path: str, file_name: str, movie_number: str, naming_rule: str):
    # 更新模式 或 读取模式
    if config.main_mode == 3 or config.main_mode == 4:
        if config.update_mode == "c" and config.success_file_rename == 0:
            return
